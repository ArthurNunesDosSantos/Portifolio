...
def deletar_flashcard(request, id):
    ...
    if not request.user.id == flashcard.user.id:
        messages.add_message(request,constants.WARNING,'Flashcard nao localizado!')
        return redirect('/flashcard/novo_flashcard')
    flashcard.delete()
...
def iniciar_desafio(request):
    if request.method == "GET":
        ...
        desafio.save( )
        # quando for um campo many to many eh necessario fazer isso
        ...
        for flashcard in flashcards:
            ...
            qtd_flashcard = qtd_flashcard + 1
        if qtd_flashcard < int(qtd_perguntas):
            messages.add_message(request,constants.WARNING,'Existem mais perguntas do que Flashcards')
            return redirect('/flashcard/iniciar_desafio')
            
        
        
        flashcards = flashcards[0: int(qtd_perguntas)]
        for f in flashcards:
        ...
        return redirect('/flashcard/listar_desafios') 
...
def listar_desafios(request):
    ...
    dificuldade_filtar = request.GET.get('dificuldade')
  
  
    if categoria_filtrar: 
        desafios = Desafio.objects.filter(user=request.user).filter(categoria = categoria_filtrar)
    if dificuldade_filtar:
        desafios = Desafio.objects.filter(user=request.user).filter(dificuldade = dificuldade_filtar)  
    
    
    return render(request,'listar_desafios.html',
                   {'desafios': desafios, 
...
def desafio(request, id):
    ...
    if request.method =='GET':
        ...
        status = Desafio.objects.get(id=id)
        if  faltantes == 0:
